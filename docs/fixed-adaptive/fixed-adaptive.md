...
Topological data analysis (TDA) is a relatively novel approach to informatics that leverages tools and principles from algebraic topology to characterize structural features of data sets or transform them into more manageable topological representations. As their theoretical foundation has been developed, TDA tools have proven to be promising for extracting novel information from high-dimensional, noisy, and complex data sets such as those found in biomedicine. To operate efficiently on such data, many TDA techniques intially employ \emph{cardinality reduction} to select a smaller subset of data points to serve as representative "landmarks" or "witnesses" to the input data. These \emph{landmark sets} are generally either chosen at random, or using the "maxmin" selection procedure to generate landmark points that are roughly evenly distributed throughout the data which can be used, for example, to construct covers comprised of sets of uniform radius. However, issues with this procedure may arise with certain types of data, such as those that vary greatly in density or have points with high multiplicities, as is common in biomedical data. In this paper, we propose an alternative procedure for construction of landmark sets, which we call "lastfirst". Unlike maxmin, lastfirst is not based on distance in terms of a raw similarity measure, but instead on a rank-order of data points to each landmark point. In this way, covers comprised of sets with uniform cardinality rather than uniform radius can be constructed. We first rigorously define the cardinality-based neighborhood constructions upon which the lastfirst algorithm is based and prove that this procedure results in the desired landmark set, then we go on to perform benchmark testing of our algorithm and compare its performance to maxmin on both simulated and real-world biomedical data. These experimental results indicate that this approach confers a number of advantages: (1) lastfirst is more general than maxmin in that it can be applied to any data on which \emph{directed} (not necessarily symmetric) pairwise distances can be computed; (2) lastfirst achieves comparable performance to maxmin on all tested data, but outperforms maxmin in terms of homology detection on simulated data with non-uniform density; (3) there may be interpretability benefits to specifying neighborhood size (as in lastfirst) instead of radius (as in maxmin) on data for which the meaning of numerical values of distance may not be intuitive, such as in biomedical contexts. As such, the lastfirst algorithm provides a viable alternative to the maxmin procedure for landmark selection and subsequent constructions which may be particularly useful on variable density data such as those commonly seen in applications.
