...
folds <- data.frame(outer = rep(seq(6L), times = 6L),
                    inner = rep(seq(6L), each = 6L))
...
evaluation <- tibble()
for (i in seq(6L)) for (j in seq(6L)) {
  print(c(i, j))
  ...
  # plot AUC across neighborhood size by weight functino!
  k_wt_aucs %>%
    as_tibble() %>%
  ...
  max_auc <- max(k_wt_aucs[seq(12L, nrow(k_wt_aucs)), ])
  k_wt_opt <- which(k_wt_aucs[seq(12L, nrow(k_wt_aucs)), ] == max_auc,
                    arr.ind = TRUE) + c(12L, 0L)
  ...
  # augment data frame
  evaluation <- bind_rows(evaluation, tibble(
    outer = i, inner = j,
    opt_auc = max_auc,
