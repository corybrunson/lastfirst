...
# subset of data for predictive modeling and identify consecutive windows
# (week 10 spans March 4-10, Wed-Tue)
covid19mx_cases %>%
...
covid19mx_subset %>%
  ...
  group_by(entidad_um, semana_ingreso, resultado) %>%
...
for (entidad in unique(covid19mx_model$entidad_um)) {
  ...
  # loop over folds
  for (i in unique(covid19mx_model$semana_ingreso)[-1L]) for (j in seq(i_folds)) {
    
    # training, optimizing, and testing indices
    train <- which(covid19mx_model$semana_ingreso == i - 1L)
    opt <- which(covid19mx_model$semana_ingreso == i &
                   covid19mx_model$inner == j)
    test <- which(covid19mx_model$semana_ingreso == i &
                    covid19mx_model$inner != j)
    
    # binary predictor and response matrices
    covid19mx_pred <- covid19mx_model %>%
      select_if(is.integer) %>%
      select(-semana_ingreso, -inner, -resultado) %>%
      as.matrix()
    ...
    test_preds <- proxy::dist(covid19mx_pred[train, ], covid19mx_pred[test, ],
      ...
      sapply(function(df) mean(covid19mx_resp[train[df$id[df$rank <= k_opt]], ]))
    ...
    for (l in seq_along(lmk_funs)) for (n_lmks in ns_lmks) {
      ...
      nbrs <- proxy::dist(covid19mx_pred[train, ], covid19mx_pred[train[lmks], ],
                          method = "cosine") %>%
        unclass() %>% as.data.frame() %>% as.list() %>% unname() %>%
        lapply(enframe, name = "id", value = "dist") %>%
        lapply(mutate, rank = rank(dist, ties.method = "min")) %>%
